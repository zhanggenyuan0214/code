...
def run_auto_migration():
    ...
    if db_path is None:
        logger.info("当前配置不是 SQLite 文件数据库，跳过自动迁移")
        return
    ...
    try:
        from app.models import TEAM_MEMBER_LIMIT
        ...
        cursor.execute(
            ...
            UPDATE teams
            SET max_members = ?
            WHERE max_members IS NULL OR max_members < 1 OR max_members > ?
            """,
            (TEAM_MEMBER_LIMIT, TEAM_MEMBER_LIMIT),
        )
        if cursor.rowcount > 0:
            logger.info(f"修正 teams.max_members 到统一上限 {TEAM_MEMBER_LIMIT}")
            migrations_applied.append("teams.max_members_capacity")
