...
# 留空时会使用安全默认值：development=False，production=True
SESSION_HTTPS_ONLY=
...
### 5. 初始化数据库（可选，推荐首次部署时执行）

应用启动时会自动执行与 `init_db.py` 相同的初始化流程，包括：
- 创建数据表
- 自动迁移数据库
- 初始化管理员密码
- 补齐默认设置（如 `proxy`、`proxy_enabled`、`log_level`）

如果你希望在启动前先手动完成这一步，可以执行：
...
Docker 配置中会将宿主机项目根目录下的 `./data/team_manage.db` 映射到容器内的同一路径策略，因此无论从哪个工作目录启动，都会落到同一个数据库文件。
...
│       ├── css/                # 样式文件
│       └── js/                 # JavaScript 文件
├── init_db.py                  # 数据库初始化脚本（复用应用启动 bootstrap）
...
默认使用 SQLite 数据库，统一数据库文件路径为项目根目录下的 `./data/team_manage.db`。配置中的相对 SQLite 路径会按项目根目录解析，不再随当前工作目录变化。旧版本遗留的 `team_manage.db` 或 `app/team_manage.db` 不会自动删除，但默认配置不再使用它们。如需使用其他数据库，请修改 `DATABASE_URL`。
...
4. `SESSION_HTTPS_ONLY`: 建议留空使用默认值；生产环境留空时会自动启用 `Secure` Cookie，不要显式设为 `False`
...
- `POST /redeem/confirm` - 直接兑换并自动分配 Team
- `GET /admin` - 管理员控制台
...
```bash
# 删除旧数据库文件
rm data/team_manage.db

# 重新初始化（或直接启动应用，由启动流程自动执行同一套初始化）
python init_db.py
```
